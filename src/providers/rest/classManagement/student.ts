...
export const getStudent = async (id: string) => {
  const response = await ClassAPI.get(`student/${id}`).json();
  return response;
};

export const createStudent = async (data: IStudent) => {
  const response = await ClassAPI.post('student', { json: data }).json();
  return response;
};

export const updateStudent = async (id: string, data: IStudent) => {
  const response = await ClassAPI.put(`student/${id}`, { json: data }).json();
  return response;
};
